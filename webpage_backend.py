...
def getPrediction_By_Date():
    ...
    if(request.args.get('date')):
        ...
        # get price 1h ahead
        prices, dataFrame =main(load_trans_model = "96-48.pkl",
               load_lstm_model = "epoch=12-step=649.ckpt",
               path_to_save_prediction = "./output/",
               choose_date = date_,
               choose_hour = time,
               devices = "cpu")
        ...
        # get prices 1d ahead
        price_1d = prices[23][0]

        # get prices 1w ahead
        price_1w = prices[-1][0]


        # dictionary_dates_prices= dataFrame.to_dict('split')
        print(dataFrame.index.tolist())
        print(dataFrame.to_dict('list')['Value'])
        values = dataFrame.to_dict('list')['Value']
    ...
    return render_template('index.html', values=values, labels=labels, date=date, price_1h=price_1h, price_1d=price_1d, price_1w=price_1w)
